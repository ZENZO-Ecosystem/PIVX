...
UniValue preparebudget(const JSONRPCRequest& request)
{
    ...
    CWalletTx wtx;
    if (!pwallet->GetBudgetSystemCollateralTX(wtx, budgetProposalBroadcast.GetHash(), useIX)) { // 50 ZNZ collateral for proposal
    ...
    pwallet->CommitTransaction(wtx, reservekey, useIX ? "ix" : "tx");
