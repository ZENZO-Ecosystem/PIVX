...
UniValue generate(const JSONRPCRequest& request)
{
    ...
    while (nHeight < nHeightEnd && !ShutdownRequested())
    {
        std::unique_ptr<CBlockTemplate> pblocktemplate(fPoS ?
                                                       CreateNewBlock(CScript(), vpwallets.front(), fPoS) :
                                                       CreateNewBlockWithKey(reservekey, vpwallets.front()));
...
UniValue setgenerate(const JSONRPCRequest& request)
{
    ...
    GenerateBitcoins(fGenerate, vpwallets.front(), nGenProcLimit);
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (pindexPrev != chainActive.Tip() ||
        ...
        pblocktemplate = CreateNewBlock(scriptDummy, vpwallets.front(), false);
        if (!pblocktemplate)
